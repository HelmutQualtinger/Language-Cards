...
- `italian`, `german`, `english`, `french` — single-word/phrase translations
- `symbol` (emoji, may be empty)
- `sentence_es`, `sentence_it`, `sentence_de`, `sentence_en`, `sentence_fr` — one example sentence per language, all five expressing the same scene. The vocabulary word's inflected form is wrapped in `**double asterisks**` (e.g. `"El **perro** corre."`) — `renderSentenceHTML()` turns that into `<u>` for display, `stripSentenceMarkers()` strips it before speaking.

When adding/editing entries, keep all five sentences semantically aligned and grammatically correct in each language (case/agreement for German, gender/elision for French, conjugation for verbs, etc.) — this was previously done by hand/agent pass and reviewed for correctness; don't regress quality by generating sentences mechanically.

## App architecture (`app.js`)

**Language model**: `state.targetLang` (`'it' | 'de' | 'en' | 'fr'`) is the single source of truth for *both* which language the quiz answers/example sentences are in *and* which language the entire GUI chrome is rendered in — the two are intentionally coupled, not independent settings. Persisted to `localStorage` under `spanish_cards_target_lang_v1`. `state.sourceLang` (`'es' | 'it' | 'de' | 'en' | 'fr'`) picks which language the quizzed word/sentence is shown *in* (persisted under `spanish_cards_source_lang_v1`); it does **not** affect UI chrome language. `TARGET_LANG_OPTIONS` (`['it','de','en','fr']`) is the restricted set target may take — Spanish can only ever be a source, never a target/UI language, since `I18N`/`WORD_TYPE_LABELS`/`GENDER_WORDS` are only translated into those four. `source !== target` is enforced by swapping the other selector's value on conflict (see `initLangSource`/`initLangTarget`).
- `LANG_META` maps a lang code (source or target) to the `WORDS_DATA` field/sentence field to quiz on — includes an `es` entry (`spanish`/`sentence_es`) alongside `it`/`de`/`en`/`fr`.
...
**Word-details reveal panel**: driven by `DETAIL_LANGS` (`['es','it','de','en','fr']`) and `el.detailRows[lang]` — a generic loop hides whichever row matches the current `state.sourceLang` (it's already visible as the question) and fills in the rest from `LANG_META[lang].field`, rather than hardcoding per-language elements.
