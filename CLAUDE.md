...
- `manifest.json` / `sw.js` / `icons/` — make the page installable as a standalone app (Android "Add to Home screen" / iOS "Add to Home Screen", no address bar or browser chrome). `sw.js` is a minimal network-first service worker over the app shell, registered from the bottom of `app.js`; it exists mainly to satisfy Chrome's installability requirement (a registered service worker with a fetch handler), with working offline reopen as a side benefit. It is deliberately network-first, not cache-first: cache-first was tried initially and shipped a bug where a fix to `app.js` landed on the server but browsers that had already installed the app kept serving the old cached `app.js` forever, surviving even a hard refresh (unlike the HTTP cache, a service worker's Cache Storage isn't cleared by cmd/ctrl+shift+R). Keep it network-first — bumping `CACHE_NAME` in `sw.js` is otherwise required on every deploy and is easy to forget. The manifest's `name`/`short_name` are static (unlike the rest of the UI, which is retranslated at runtime by `applyUILanguage()`) since the OS reads them once at install time. `icons/icon-192.png` and `icons/icon-512.png` are screenshots of `icons/icon-source.html` (same 🎴 logo/gradient as the page header) rendered at exact pixel size via headless Chrome (`--window-size=512,512 --screenshot=...`), not hand-drawn — edit that HTML and re-render (then downscale with `sips -z 192 192`) if the logo ever needs to change, rather than hand-editing the PNGs.
