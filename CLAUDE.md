...
`NETWORK_ONLY_LANGS` (currently `{'hu', 'ga'}`) forces a language to always use the network TTS fallback, skipping `pickVoiceForLang` entirely — for languages whose macOS voice is listed in `getVoices()` but commonly isn't actually downloaded (reported: Hungarian "Tünde" silently played back in the Mac's default system voice instead, with no error event at all, since there's no Web Speech API to detect an undownloaded voice), or that macOS ships no native voice for at all (Irish/`ga-IE` — macOS only has Irish-*accented English* voices like Moira/Fiona, which are `en-IE`, not the Irish language itself). Add a language here if the same symptom (a language's audio "sounds like [wrong language]", or simply doesn't exist locally) gets reported again.
